...
public class OFileClassic implements OFile {
  ...
  @Override
  public void read(long offset, ByteBuffer data) throws IOException {
    int attempts = 0;
    ...
    while (true) {
      try {
        acquireReadLock();
        try {
          offset = checkRegions(offset, data.limit() - data.position());
  ...
  public void lock() throws IOException {
    ...
    try {
      for (int i = 0; i < LOCK_MAX_RETRIES; ++i) {
        ...
        if (fileLock == null)
          throw new OLockException("File '" + osFile.getPath()
