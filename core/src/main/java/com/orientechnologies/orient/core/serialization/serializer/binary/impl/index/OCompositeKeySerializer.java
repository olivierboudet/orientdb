...
public class OCompositeKeySerializer implements OBinarySerializer<OCompositeKey>, OStreamSerializer {
  ...
  @Override
  public void serializeInByteBuffer(OCompositeKey compositeKey, ByteBuffer byteBuffer, final int offset, Object... hints) {
    final OType[] types = getKeyTypes(hints);

    final List<Object> keys = compositeKey.getKeys();
    final int keysSize = keys.size();

    byteBuffer.position(offset + OIntegerSerializer.INT_SIZE);
    byteBuffer.putInt(keysSize);
    ...
    for (int i = 0; i < keys.size(); i++) {
      ...
      binarySerializer.serializeInByteBuffer(key, byteBuffer, bufferPos);
      byteBuffer.position(binarySerializer.getObjectSize(key) + bufferPos);
    }

    byteBuffer.putInt(offset, byteBuffer.position() - offset);
  }
  ...
  @Override
  public OCompositeKey deserializeFromByteBufferObject(ByteBuffer byteBuffer, final int offset) {
    final OCompositeKey compositeKey = new OCompositeKey();

    byteBuffer.position(offset + OIntegerSerializer.INT_SIZE);
    final int keysSize = byteBuffer.getInt();

    final OBinarySerializerFactory factory = OBinarySerializerFactory.getInstance();
    for (int i = 0; i < keysSize; i++) {
      final byte serializerId = byteBuffer.get();
      OBinarySerializer<Object> binarySerializer = (OBinarySerializer<Object>) factory.getObjectSerializer(serializerId);
      final int bufferPos = byteBuffer.position();
      final Object key = binarySerializer.deserializeFromByteBufferObject(byteBuffer, bufferPos);
      byteBuffer.position(bufferPos + binarySerializer.getObjectSizeInByteBuffer(byteBuffer, bufferPos));
      compositeKey.addKey(key);
    }
  ...
  @Override
  public OCompositeKey deserializeFromByteBufferObject(OWALChangesTree.BufferWrapper wrapper, int offset) {
    final OCompositeKey compositeKey = new OCompositeKey();
    ...
    for (int i = 0; i < keysSize; i++) {
      ...
      OBinarySerializer<Object> binarySerializer = (OBinarySerializer<Object>) factory.getObjectSerializer(serializerId);
      final Object key = binarySerializer.deserializeFromByteBufferObject(wrapper, offset);
      compositeKey.addKey(key);
