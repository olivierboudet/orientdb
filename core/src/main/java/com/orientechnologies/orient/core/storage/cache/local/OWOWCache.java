...
public class OWOWCache extends OAbstractWriteCache implements OWriteCache, OCachePointer.WritersListener, OWOWCacheMXBean {
  ...
  private OCachePointer cacheFileContent(final long fileId, final int intId, final long pageIndex, final boolean addNewPages)
    ...
    if (fileClassic.getFileSize() >= endPosition) {
      byteBuffer = byteBuffersPool.acquire();
      byteBuffer.limit(pageSize);
      byteBuffer.position(0);
      fileClassic.read(startPosition, byteBuffer);

      dataPointer = new OCachePointer(byteBuffersPool, byteBuffer, lastLsn, fileId, pageIndex);

    } else if (addNewPages) {
      byteBuffer = byteBuffersPool.acquire(true);
  ...
  private OCachePointer[] cacheFileContent(final long fileId, final int intId, final long pageIndex,
    ...
    final OPageCacheByteBuffersPool byteBuffersPool = OPageCacheByteBuffersPool.instance();
    final ByteBuffer[] byteBuffers = new ByteBuffer[loadPages];
    for (int i = 0; i < byteBuffers.length; i++) {
      byteBuffers[i] = byteBuffersPool.acquire(false);
    }

    // READ ALL THE PAGES IN JUST ONE CALL
    fileClassic.read(startPosition, byteBuffers);

    for (int i = 0; i < loadPages; ++i) {
      dataPointers[i] = new OCachePointer(byteBuffersPool, byteBuffers[i], lastLsn, fileId, pageIndex + i);
  ...
  private void flushPage(final int fileId, final long pageIndex, final ByteBuffer byteBuffer) throws IOException {
    if (writeAheadLog != null) {
      ...
      while (entriesIterator.hasNext()) {
        ...
        try {
          ...
          if (group.recencyBit && weakLockMode)
            ...
            try {
              flushPage(entry.fileId, entry.pageIndex, pagePointer.getByteBuffer());

              final OLogSequenceNumber flushedLSN = ODurablePage.getLogSequenceNumberFromPage(pagePointer.getByteBuffer());
      ...
      while (entriesIterator.hasNext()) {
        ...
        try {
          if (group.recencyBit && weakLockMode)
            ...
            try {
              flushPage(pagedKey.fileId, pagedKey.pageIndex, pagePointer.getByteBuffer());
