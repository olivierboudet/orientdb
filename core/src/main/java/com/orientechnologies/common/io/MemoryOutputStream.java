...
public class MemoryOutputStream extends OutputStream {
   ...
   *           if {@code minCapacity < 0}. This is interpreted as a request for the unsatisfiably large capacity
  ...
  /**
   * The maximum size of array to allocate. Some VMs reserve some header words in an array. Attempts to allocate larger arrays may
  ...
  /**
   * Writes the complete contents of this byte array output stream to the specified output stream argument, as if by calling the
   * output stream's write method using <code>out.write(buf, 0, count)</code>.
   *
   * @param out
  ...
  /**
   * Resets the <code>count</code> field of this byte array output stream to zero, so that all currently accumulated output in the
   * output stream is discarded. The output stream can be used again, reusing the already allocated buffer space.
   *
  ...
  /**
   * Converts the buffer's contents into a string by decoding the bytes using the named {@link java.nio.charset.Charset charset}.
   * The length of the new <tt>String</tt> is a function of the charset, and hence may not be equal to the length of the byte array.
   *
   * <p>
  ...
  /**
   * Creates a newly allocated string. Its size is the current size of the output stream and the valid contents of the buffer have
   * been copied into it. Each character <i>c</i> in the resulting string is constructed from the corresponding element <i>b</i> in
   * the byte array such that: <blockquote>
   * 
   * <pre>
   ...
   * </blockquote>
   *
   * @deprecated This method does not properly convert bytes into characters. As of JDK&nbsp;1.1, the preferred way to do this is
   *             via the <code>toString(String enc)</code> method, which takes an encoding-name argument, or the
